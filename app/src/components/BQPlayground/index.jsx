...
const TABLE_SIZES = {
  ...
  centro_custos: { rows: 30, sizeMB: 0.01 },
  plano_contas: { rows: 80, sizeMB: 0.02 },
...
function translateBQtoSQLite(sql) {
  ...
  s = s.replace(/CAST\s*\(\s*(\w+(?:\.\w+)?)\s+AS\s+STRING\b/gi, "CAST($1 AS TEXT");
  s = s.replace(/CAST\s*\(\s*(\w+(?:\.\w+)?)\s+AS\s+INT64\b/gi, "CAST($1 AS INTEGER");
  s = s.replace(/CAST\s*\(\s*(\w+(?:\.\w+)?)\s+AS\s+FLOAT64\b/gi, "CAST($1 AS REAL");
